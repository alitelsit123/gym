...
@section('body')
@php
$schedule = \App\Models\ScheduleNutrition::whereTrainer_member_id($member->id)->get();
@endphp
<div class="card">
  <div class="card-body">
    <div class="d-flex align-items-center justify-content-between">
      <h4>Manage Jadwal Nutrisi {{$member->member->name}}</h4>
      ...
      <div class="modal fade" id="exampleModal-2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{url('trainer/member/store_nutrition')}}" method="post" enctype="multipart/form-data">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                      ...
                      <select name="type" id="" class="form-control">
                        @foreach (['wajib','tambahan'] as $rowT)
                        <option value="{{$rowT}}">{{$rowT}}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
    ...
    <table class="table table-stripped mt-2">
      ...
      <tbody>
        ...
        <tr>
            ...
            <div class="modal fade" id="exampleModal-{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-{{$row->id}}" aria-hidden="true">
              <div class="modal-dialog" role="document">
                  <div class="modal-content">
                      ...
                      <form action="{{url('trainer/member/update_nutrition')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                          ...
                          <div class="form-group mb-3">
                            ...
                            <select name="type" id="" class="form-control">
                              @foreach (['wajib','tambahan'] as $rowT)
                              <option value="{{$rowT}}" @if($row->type == $rowT) selected @endif>{{$rowT}}</option>
                              @endforeach
                            </select>
                          </div>
                        </div>
          ...
          <!-- Button trigger modal -->
          <a href="{{url('trainer/member/destroy_nutrition')}}?id={{$row->id}}" class="btn btn-danger btn-sm btn-block" onclick="return confirm('Yakin ingin hapus ?');">
            Hapus
          </a>
          </td>
        </tr>
