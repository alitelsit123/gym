                    ...
                    <form action="{{url('akuntan/approved_payment')}}" method="get" class="payment-{{$row->id}}">
                      ...
                      <div class="modal-body">
                        <div class="form-group mb-3">
                          ...
                          <input type="string" value="Rp {{number_format($row->type->price)}}" id="" class="form-control" disabled />
                        </div>
                        <div class="form-group mb-3">
                        ...
                        $(document).ready(function() {
                          $('.payment_totalm{{$row->id}}').keyup(function() {
                            $('.kembalim{{$row->id}}').val(`Rp ${($(this).val() - {{$row->type->price}}).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 0})}`)
                          })
                        })
                        </script>
