...
<table class="table table-stripped mt-2 to">
  ...
  <tbody>
    ...
    @php
    $total = $total+(Str::ucfirst($row->type) == 'Membership' ? $row->membershipType->{'price'}:(Str::ucfirst($row->type) == 'Product' ? ($row->details()->sum('sub_amount') > 0 ? $row->details()->sum('sub_amount') ?? 0:0):0));
    @endphp
    {{-- @dd($row->toArray()) --}}
    <tr>
      <th scope="row">{{$k+1}}</th>
